...
bool Habitacion::estaDisponible(Fecha &fechaInicio, Fecha &fechaFin){
	for (size_t i = 0; i < Reservas.size(); ++i) {
		if (!(Reservas[i]->getFechaEntrada() >= fechaFin || fechaInicio >= Reservas[i]->getFechaSalida())) {
			return false;
		}
	}
	return true;
